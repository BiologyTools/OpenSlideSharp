    ...
    public abstract class SlideSourceBase : ISlideSource, IDisposable
    {
        ...
        public static ISlideSource Create(string source, bool enableCache = true)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine("return factory.Invoke(source, enableCache);");
            ...
            Console.WriteLine("Failed to open slide.");
            return null;
        }
        #endregion
        ...
        protected MemoryCache<Bitmap> _bgraCache = new MemoryCache<Bitmap>();
        ...
        public virtual byte[] GetSlice(SliceInfo sliceInfo)
        {
            if (sliceInfo.Extent.Intersects(Schema.Extent) && sliceInfo.Resolution != 0)
            {
                ...
                Func<TileInfo, Bitmap> getOrInsterCache = new Func<TileInfo, Bitmap>(_ =>
                 {
                     var cache = _bgraCache.Find(_.Index);
                     if (cache == null)
                     {
                         //cache = new Mat(new Size(256,256), DepthType.Cv8U, 3);
                         byte[] rgba = GetTile(_);
                         cache = new Bitmap(256, 256, PixelFormat.Format24bppRgb, rgba, new ZCT(), "");
                ...
                return ImageUtil.Join(tiles, srcPixelExtent, new Extent(0, 0, dstPixelWidth, dstPixelHeight)).Bytes;
            }
            return null;
        }
