
![openslide](./openslide_logo.png)

[![NuGet version (OpenSlideGTK)](https://img.shields.io/nuget/v/OpenSlideGTK.svg?style=flat-square)](https://www.nuget.org/packages/OpenSlideGTK/2.6.0)

# OpenSlideGTK
.NET10 cross-platform x64 & arm64, Windows, Linux, Mac, bindings for OpenSlide (http://openslide.org/). As well as cross platform .NET10 slide viewer.  

Thank you to @IOL0ol1 for his work on [OpenSlideSharp](https://github.com/IOL0ol1/OpenSlideSharp).

Thank you to @yigolden for his work on [OpenSlideNET](https://github.com/yigolden/OpenSlideNET).

Nuget    
```ps
Install-Package OpenSlideGTK -Version 2.7.0
```

## Index

1.  [OpenSlideGTK](/src/OpenSlideGTK)    
    Openslide wrapper, including DeepZoomGenerator, but no native *.dlls.
2.  [SlideViewer](/example/SlideViewer/)
    .NET10 Cross-platform example project for viewing slide with GTK.
