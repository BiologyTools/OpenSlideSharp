...
namespace OpenSlideSharp
{
    ...
    public partial class OpenSlideImage : IDisposable
    {
        ...
        {
            var openslide = Directory.GetParent(Assembly.GetCallingAssembly().Location)?.FullName;
        ...
        public unsafe byte[] ReadRegion(int level, long x, long y, long width, long height)
        {
            ...
            CheckIfThrow(0);
            return Bitmap.GetRGB24Data((int)width, (int)height, PixelFormat.Format32bppArgb, data).Bytes;
        }
        ...
        public unsafe bool TryReadRegion(int level, long x, long y, long width, long height, out byte[] data)
        {
            ...
            NativeMethods.ReadRegion(Handle, data, x, y, level, width, height);
            string er = NativeMethods.GetError(Handle);
            if (er != null)
            {
