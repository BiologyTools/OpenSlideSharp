...
namespace OpenSlideGTK
{
    public class Stitch
    {
        ...
        public List<Tuple<Extent, CudaDeviceVariable<byte>>> gpuTiles = new List<Tuple<Extent, CudaDeviceVariable<byte>>>();
        ...
        public byte[] StitchImages(int pxwidth, int pxheight, double x, double y)
        {
            try
            {
                if (!initialized)
