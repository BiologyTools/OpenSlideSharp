...
namespace OpenSlideSharp.BruTile
{
    public class ImageUtil
    {
        ...
        public static unsafe byte[] GetJpeg(byte[] bgraBytes, int bytesPerPixel, int bytesPerLine, int width, int height, int dstWidth = 0, int dstHeight = 0, int? quality = null, uint background = 0xFFFFFFFF)
        {
            ...
            {
                using (var src = new Mat(new Size(height, width), DepthType.Cv8U, bytesPerPixel, (IntPtr)scan0, bytesPerLine))
                {
                    /*
        ...
        byte[] bt = new byte[width * 4 * ]
        Image<Bgr, byte> img = src.ToImage<Bgr, byte>();
        byte[] bts = img.ToJpegData();
        return bts;
        /*
        }
        else  // fill 
...
*/
        ...
        */
        ...
        public static Bitmap Join(IEnumerable<Tuple<Extent, Bitmap>> srcPixelTiles, Extent srcPixelExtent, Extent dstPixelExtent)
        {
            ...
            if (canvas.Height != canvasWidth || canvas.Width != canvasHeight)
            {
