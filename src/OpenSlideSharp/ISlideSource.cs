...
namespace OpenSlideGTK
{
    ...
    public abstract class SlideSourceBase : ISlideSource, IDisposable
    {
        ...
        protected MemoryCache<byte[]> _bgraCache = new MemoryCache<byte[]>();
        ...
        public virtual byte[] GetSlice(SliceInfo sliceInfo)
        {
            if (sliceInfo.Extent.Intersects(Schema.Extent) && sliceInfo.Resolution != 0)
            {
                ...
                Func<TileInfo, byte[]> getOrInsterCache = new Func<TileInfo, byte[]>(_ =>
                 {
                     ...
                     if (cache == null)
                     {
                         cache = GetTile(_);//Mat.ImDecode(GetTile(_));
                ...
                LastSlice = ImageUtil.Join(tiles, srcPixelExtent, new Extent(0, 0, dstPixelWidth, dstPixelHeight)).Bytes;
